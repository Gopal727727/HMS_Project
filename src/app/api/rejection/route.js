...
export async function POST(req) {
    try {
        ...
        if (type === 'room') {
            updateQuery = `UPDATE roombook SET approval = 'rejected' WHERE id = $1`;
            emailSubject = 'Room Booking Confirmation';
            emailText = 'Sorry, Your room booking is rejected!!';
        } else if (type === 'event') {
            updateQuery = `UPDATE eventbook SET approval = 'rejected' WHERE id = $1`;
            emailSubject = 'Event Booking Confirmation';
            emailText = 'Sorry, Your event booking is rejected!!';
        }
